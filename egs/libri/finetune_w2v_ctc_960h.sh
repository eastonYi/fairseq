...
SAVE_DIR=exp/finetune_w2v_ctc_960h
DATA_DIR=data/light/subword
label_type=subword

TOKENIZERS_PARALLELISM=false CUDA_VISIBLE_DEVICES=$gpu fairseq-train $DATA_DIR \
--train-subset train_960h --valid-subset dev_clean --criterion ctc --best-checkpoint-metric uer \
...
--update-freq 1 --log-interval 5000 --validate-interval 1 --log-format simple --save-interval 1
