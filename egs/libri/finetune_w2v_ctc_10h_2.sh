...
SAVE_DIR=exp/finetune_w2v_ctc_10h_2
DATA_DIR=data/light
BERT='bert-base-uncased'
label_type=word

TOKENIZERS_PARALLELISM=false CUDA_VISIBLE_DEVICES=$gpu fairseq-train $DATA_DIR \
--train-subset train_10h --valid-subset dev_clean --criterion ctc_v2 --best-checkpoint-metric uer \
--save-dir $SAVE_DIR --tensorboard-logdir $SAVE_DIR --bert-name $BERT \
--num-workers 4 --max-update 80000 --task audio_cif_bert --arch wav2vec_ctc \
