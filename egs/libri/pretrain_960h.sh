...
--save-dir $SAVE_DIR --tensorboard-logdir $SAVE_DIR --num-workers 4 --task audio_pretraining --criterion wav2vec --arch wav2vec2 \
--train-subset train_960h --valid-subset dev_clean \
...
--max-sample-size 200000 --min-sample-size 32000 --dropout 0.1 --attention-dropout 0.1 --weight-decay 0.01 \
--max-tokens 900000 --max-update 400000 --skip-invalid-size-inputs-valid-test --ddp-backend no_c10d \
--log-format simple --log-interval 500 --update-freq 1 --save-interval 1
