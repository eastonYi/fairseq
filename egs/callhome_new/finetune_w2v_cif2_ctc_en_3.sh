...
GPT='gpt2'
SAVE_DIR=exp/finetune_w2v_cif2_ctc_en_3
...
--train-subset train --valid-subset train_100 --no-epoch-checkpoints \
--labels $label_type --num-workers 4 --max-update 80000 \
--lambda-ctc 1.0 --lambda-qua 0.5 \
--arch w2v_cif --task audio_cif_gpt2 --criterion ctc_cif_gpt2 --best-checkpoint-metric uer \
