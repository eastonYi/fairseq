gpu=$1
SAVE_DIR=exp/finetune_w2v_seq2seq_lm_coldfusion_layer1_ma
...
--arch wav2vec_seq2seq_lm_coldfusion --task audio_seq2seq --criterion ls_ce_acc --best-checkpoint-metric uer \
--w2v-path $W2V_PATH --decoder seq2seq_decoder \
--decoder-embed-dim 768  --lm-path $LM_PATH --dim-hidden-mixer 1024 \
--decoder-ffn-embed-dim 768 --decoder-layers 1 --decoder-layerdrop 0.0 \
...
--freeze-lm-finetune-updates 20000 --validate-after-updates 5000  --validate-interval 1 \
...
--attention-dropout 0.0 --max-tokens 700000 --seed 2337 --ddp-backend no_c10d --update-freq 2 \
