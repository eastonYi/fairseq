...
SAVE_DIR=exp/finetune_w2v_ctc_cif2_bert_base_ma_new_4
...
--lambda-ctc 0.5 --lambda-alpha 0.0 --lambda-qua 0.2 --lambda-embedding 0.05 --lambda-lm 0.2 \
--arch wav2vec_ctc_cif2_bert --task audio_cif_bert --criterion ctc_cif_bert --best-checkpoint-metric uer \
--w2v-path $W2V_PATH --bert-name $BERT --infer-threash 1.0 \
...
--feature-grad-mult 0.0 --freeze-finetune-updates 500 --gold-updates 8000 --gold-rate-range '(0.9, 0.2)' \
