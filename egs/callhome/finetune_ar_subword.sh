gpu=$1
SAVE_DIR=exp/finetune_ar_subword_2
W2V_PATH=../libri/wav2vec2_small.pt
DATA_DIR=data/ar/subword
label_type=subword
...
--train-subset train --valid-subset dev_ar --no-epoch-checkpoints --best-checkpoint-metric uer \
--num-workers 2 --max-update 80000 --sentence-avg --task audio_ctc --arch wav2vec_ctc --w2v-path $W2V_PATH \
--labels $label_type --apply-mask --mask-selection static --mask-other 0 --mask-length 10 --mask-prob 0.5 \
--layerdrop 0.1 --mask-channel-selection static --mask-channel-other 0 --mask-channel-length 64 \
--mask-channel-prob 0.5 --zero-infinity \
--feature-grad-mult 0.0 --freeze-finetune-updates 3000 --validate-after-updates 10000  --validate-interval 10 \
...
--log-interval 40 --log-format simple --save-interval 20
