gpu=$1
SAVE_DIR=exp/finetune_w2v_cif_rnn_ma_onlydev
...
--train-subset dev --valid-subset dev --no-epoch-checkpoints \
--labels $label_type --num-workers 4 --max-update 80000 \
--arch wav2vec_cif_rnn --task audio_cif --criterion qua_ce_acc --best-checkpoint-metric acc \
...
--freeze-finetune-updates 2000 --teacher-forcing-updates 10000 \
...
--warmup-steps 1000 --hold-steps 20000 --decay-steps 30000 --final-lr-scale 0.05 \
