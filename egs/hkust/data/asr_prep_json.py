...
def process_sample(aud_path, lable, utt_id, tgt_dict):
    import torchaudio
    input = {}
    output = {}
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_cpu) as executor:
        future_to_sample = {executor.submit(process_sample, s.aud_path, labels[s.utt_id], s.utt_id, tgt_dict): s for s in samples}
        for future in concurrent.futures.as_completed(future_to_sample):
            try:
                ...
                utts.update(data)
    json.dump({"utts": utts}, args.output, indent=2, ensure_ascii=False)
