SAVE_DIR=exp/wav2vec2_base_finetune_hkust_all_word_phone_v1
W2V_PATH=exp/wav2vec2_base_pretrain/checkpoint_best.pt
DATA_DIR=data/hkust_all/train/word_phone_v1
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python train.py $DATA_DIR --save-dir $SAVE_DIR --tensorboard-logdir $SAVE_DIR --fp16 \
--wer-args '("data/hkust_all/train/word_phone_v1/hkust.arpa.bin","data/hkust_all/train/word_phone_v1/lexicon.txt",2,-1)' \
--post-process letter --train-subset train --valid-subset "valid" --no-epoch-checkpoints --best-checkpoint-metric wer --num-workers 4 \
--max-update 100000 --sentence-avg --task audio_pretraining --arch wav2vec_ctc --w2v-path $W2V_PATH \
